...
const CarregamentoDist = (props)=>{
    ...
    const posicao = (props.mag<0)?1.50:1.15
    ...
    return (<>
        <text x={props.start+(props.comprimento-3*12)/2} y={147.5-props.mag*posicao*props.escala} fontSize={12}>{`${props.mag.toString().replace(".",",")} kN/m`}</text>
        ...
        {quantidade_arrow.map((item,index)=>{return <Arrow key ={index} x1={item} y1={155} x2={item} y2={147-props.mag*props.escala} mag={props.mag*props.escala}></Arrow> })}
